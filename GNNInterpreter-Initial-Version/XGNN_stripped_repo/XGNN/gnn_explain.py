...
class gnn_explain():
   ...
   #$     self.mol = Chem.RWMol()  ### keep a mol obj, to check if the graph is valid 
        ...
        self.color= {0:'orange', 1:'pink', 2:'green', 3:'blue', 4:'aqua', 5:'red', 6:'lime'}
        # self.color= {0:'g', 1:'r', 2:'b', 3:'c', 4:'m', 5:'w', 6:'y'}
        self.max_poss_degree = {0: 4, 1: 5, 2: 2, 3: 1, 4: 7, 5:7, 6: 5}
        

    def train(self, log_file_train=None, log_file_probs=None, classes=0, seed=0, startmol=0, maxnodes=0, onlyLogTrainTime=False):
        ####given the well-trained model
        ### Load the model
        print(os.getcwd())

        checkpoint_path = os.path.abspath('XGNN_stripped_repo/XGNN/checkpoint/ckpt.pth')
        ...
        for i in range(self.max_iters):
            ...
            for j in range(self.max_step):
                ...
                if(n>self.max_node):
                    break
                self.graph_old = copy.deepcopy(self.graph) # starting graph (default = one carbon atom)
...
#                   #print(start_logits_ori)
        ...
        if log_file_train:
            # Log train time
            log_file_train.write(f"{seed}\t{classes}\t{self.max_iters}\t{elapsed_time}\n")
        ...
        if onlyLogTrainTime == False:
            if log_file_probs:
                # Log probabilities
                log_file_probs.write(f"{seed}\t{classes}\t{prob}\n")

            # Log explanation graph
            folder_path = "XGNNplots"
            file_name = f"seed{seed}_class{classes}_startMol{startmol}_maxNodes{maxnodes}.png"

            full_path = os.path.join(folder_path, file_name)
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)

            self.graph_draw(self.graph, full_path)

        # print(prob)


    def graph_draw(self, graph, filename=None):
        ...
        for n in attr:
            ...
            color_array.append(self.color[attr[n]])
           
     #   labels=dict((n,) for n in attr)
        nx.draw(graph,labels=labels, node_color=color_array)

        # Save plot in folder if filename provided, otherwise show plot
        if filename:
    ...
    def graph_reset(self, label=0):
        # Set initial graph
        self.graph.clear()
        self.graph.add_node(0, label=label)  #self.dict = {0:'C', 1:'N', 2:'O', 3:'F', 4:'I', 5:'Cl', 6:'Br'}
