from gnninterpreter import *

import random
from collections import defaultdict, namedtuple
from itertools import combinations_with_replacement

import numpy as np
import pandas as pd
# import seaborn as sns
import matplotlib.pyplot as plt
# import plotly.express as px

from tqdm.auto import tqdm, trange

import torch
from torch import nn
import torch_geometric as pyg
from torchmetrics import F1Score
import time
import os

def seed_all(seed):
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    torch.backends.cudnn.deterministic = True

def main():
    # Get 5 random seeds
    seeds = list(range(5))

    # Create 'logs' directory (if it doesn't exist)
    logs_directory = "logs"
    if not os.path.exists(logs_directory):
        os.makedirs(logs_directory)

    cyclicitylog(seeds)

def cyclicitylog(seeds):
    with open("logs/train_times_cyclicity.txt", "w") as log_file_train, open("logs/class_probs_cyclicity.txt", "w") as log_file_probs:
        # Write header
        log_file_train.write("Seed\tClass\tTrain Time\n")
        log_file_probs.write("Seed\tClass\tMean\tStd\n")

        for _, seed in enumerate(seeds):
            # Initialize dataset
            cyclicity = CyclicityDataset(seed=seed)

            # Seed all
            seed_all(seed)

            # Load model
            kwargs = dict(node_features=len(cyclicity.NODE_CLS),
                        edge_features=len(cyclicity.EDGE_CLS),
                        num_classes=len(cyclicity.GRAPH_CLS),
                        hidden_channels=32)
            model = NNConvClassifier(**kwargs)
            model.load_state_dict(torch.load('models_checkpoints/cyclicity.pt'))

            # Generate average embedding
            embeds = [[] for _ in range(len(cyclicity.GRAPH_CLS))]
            with torch.no_grad():
                for data in tqdm(cyclicity):
                    embeds[data.y.item()].append(model.eval()(pyg.data.Batch.from_data_list([data]))["embeds"].numpy())
            mean_embeds = [torch.tensor(np.concatenate(e).mean(axis=0)) for e in embeds]

            trainer = {}
            sampler = {}

            # Class 0 (red cyclic)
            redCyclic(cyclicity, mean_embeds, model, trainer, sampler, seed, log_file_train, log_file_probs)
            # Class 1 (green cyclic)
            # seed_all(seed)
            # greenCyclic(cyclicity, mean_embeds, model, trainer, sampler, seed, log_file_train, log_file_probs)
            # Class 2 (acyclic)
            # seed_all(seed)
            # aCyclic(cyclicity, mean_embeds, model, trainer, sampler, seed, log_file_train, log_file_probs)

    return

def redCyclic(cyclicity, mean_embeds, model, trainer, sampler, seed, log_file_train, log_file_probs):
    # Class 0 (red cyclic)
    classes = 0
    sampler[classes] = s = GraphSampler(max_nodes=20,
                                        num_edge_cls=len(cyclicity.EDGE_CLS),
                                        temperature=0.15,
                                        learn_node_feat=False,
                                        learn_edge_feat=True)

    criterion = nn.Sequential(
        WeightedCriterion([
            dict(key="logits", criterion=ClassScoreCriterion(class_idx=classes), mode="maximize", weight=1),
            dict(key="embeds", criterion=EmbeddingCriterion(target_embedding=mean_embeds[classes]), weight=1),
        ]),
        NormPenalty(lambda: s.omega, order=1, weight=10), # L1 penalty on omega, encourage uncertainty
        NormPenalty(lambda: s.omega, order=2, weight=5), # L2 penalty on omega, avoid extreme probabilities
        # NormPenalty(lambda: s.xi, order=1, weight=0), # L1 penalty on xi
        # NormPenalty(lambda: s.xi, order=2, weight=0), # L2 penalty on xi
        NormPenalty(lambda: s.eta, order=1, weight=1), # L1 penalty on eta
        NormPenalty(lambda: s.eta, order=2, weight=1), # L2 penalty on eta
        budget := BudgetPenalty(lambda: s.theta, budget=15, order=2, beta=0.5, weight=200), # Budget penalty on theta, encourage sparsity
        # CrossEntropyPenalty(lambda: s.theta, weight=0), # Element-wise entropy penalty on theta, encourage discreteness
        KLDivergencePenalty(lambda: tuple(s.theta_pairs), binary=True, weight=5), # Pair-wise cross entropy on E, encourage connectivity
    )

    optimizer = torch.optim.SGD(s.parameters(), lr=1)
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=1)

    # construct graph sampler trainer
    trainer[classes] = Trainer(sampler=s,
                    discriminator=model,
                    criterion=criterion,
                    optimizer=optimizer,
                    scheduler=scheduler,
                    dataset=cyclicity,
                    k_samples=16,
                    seed=seed,
                    classes=classes)
    
    def penalty_cond(out, trainer, boundary_cls=classes):
        return out["probs"][0, classes].item() > 0.9
    def break_cond(out, trainer, boundary_cls=classes):
        return all([
            penalty_cond(out, trainer, boundary_cls),
            trainer.sampler.expected_m < 15
        ])
    
    # Train and log time
    trainer[0].train(2000, dynamic_penalty=budget, penalty_cond=penalty_cond, break_cond=break_cond, log_file=log_file_train)

    # Evaluate and record class probabilities and generated explanation graphs
    # trainer[classes].evaluate(threshold=0.5, show=False, connected=True, log=True)
    trainer[0].evaluateAndLog(threshold=0.5, log_file_probs=log_file_probs)

def greenCyclic(cyclicity, mean_embeds, model, trainer, sampler, seed, log_file_train, log_file_probs):
    # Class 1 (green cylic)
    classes = 1
    sampler[classes] = s = GraphSampler(max_nodes=20,
                                        num_edge_cls=len(cyclicity.EDGE_CLS),
                                        temperature=0.15,
                                        learn_node_feat=False,
                                        learn_edge_feat=True)

    criterion = nn.Sequential(
        WeightedCriterion([
            dict(key="logits", criterion=ClassScoreCriterion(class_idx=classes), mode="maximize", weight=1),
            dict(key="embeds", criterion=EmbeddingCriterion(target_embedding=mean_embeds[classes]), weight=1),
        ]),
        NormPenalty(lambda: s.omega, order=1, weight=10), # L1 penalty on omega, encourage uncertainty
        NormPenalty(lambda: s.omega, order=2, weight=5), # L2 penalty on omega, avoid extreme probabilities
        # NormPenalty(lambda: s.xi, order=1, weight=0), # L1 penalty on xi
        # NormPenalty(lambda: s.xi, order=2, weight=0), # L2 penalty on xi
        NormPenalty(lambda: s.eta, order=1, weight=1), # L1 penalty on eta
        NormPenalty(lambda: s.eta, order=2, weight=1), # L2 penalty on eta
        budget := BudgetPenalty(lambda: s.theta, budget=15, order=2, beta=0.5, weight=200), # Budget penalty on theta, encourage sparsity
        # CrossEntropyPenalty(lambda: s.theta, weight=0), # Element-wise entropy penalty on theta, encourage discreteness
        KLDivergencePenalty(lambda: tuple(s.theta_pairs), binary=True, weight=5), # Pair-wise cross entropy on E, encourage connectivity
    )
    optimizer = torch.optim.SGD(s.parameters(), lr=1)
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=1)

    # construct graph sampler trainer
    trainer[classes] = Trainer(sampler=s,
                    discriminator=model,
                    criterion=criterion,
                    optimizer=optimizer,
                    scheduler=scheduler,
                    dataset=cyclicity,
                    k_samples=16,
                    seed=seed,
                    classes=classes)
    
    def penalty_cond(out, trainer, boundary_cls=classes):
        return out["probs"][0, classes].item() > 0.9
    def break_cond(out, trainer, boundary_cls=classes):
        return all([
            penalty_cond(out, trainer, boundary_cls),
            trainer.sampler.expected_m < 15
        ])

    # Train and log time
    trainer[1].train(2000, dynamic_penalty=budget, penalty_cond=penalty_cond, break_cond=break_cond, log_file=log_file_train)
    # Evaluate and record class probabilities and generated explanation graphs
    # trainer[1].evaluate(threshold=0.5, show=True, connected=True, log=True)
    trainer[1].evaluateAndLog(threshold=0.5, log_file_probs=log_file_probs)

def aCyclic(cyclicity, mean_embeds, model, trainer, sampler, seed, log_file_train, log_file_probs):
    # Class 2 (acyclic)
    classes = 2
    sampler[classes] = s = GraphSampler(max_nodes=20,
                                        num_edge_cls=len(cyclicity.EDGE_CLS),
                                        temperature=0.1,
                                        learn_node_feat=False,
                                        learn_edge_feat=True)

    criterion = nn.Sequential(
        WeightedCriterion([
            dict(key="logits", criterion=ClassScoreCriterion(class_idx=classes), mode="maximize", weight=1),
            dict(key="embeds", criterion=EmbeddingCriterion(target_embedding=mean_embeds[classes]), weight=1),
        ]),
        NormPenalty(lambda: s.omega, order=1, weight=10), # L1 penalty on omega, encourage uncertainty
        NormPenalty(lambda: s.omega, order=2, weight=5), # L2 penalty on omega, avoid extreme probabilities
        # NormPenalty(lambda: s.xi, order=1, weight=0), # L1 penalty on xi
        # NormPenalty(lambda: s.xi, order=2, weight=0), # L2 penalty on xi
        NormPenalty(lambda: s.eta, order=1, weight=1), # L1 penalty on eta
        NormPenalty(lambda: s.eta, order=2, weight=1), # L2 penalty on eta
        budget := BudgetPenalty(lambda: s.theta, budget=15, order=2, beta=0.5, weight=200), # Budget penalty on theta, encourage sparsity
        # CrossEntropyPenalty(lambda: s.theta, weight=0), # Element-wise entropy penalty on theta, encourage discreteness
        KLDivergencePenalty(lambda: tuple(s.theta_pairs), binary=True, weight=5), # Pair-wise cross entropy on E, encourage connectivity
    )
    optimizer = torch.optim.SGD(s.parameters(), lr=1)
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=1)

    # construct graph sampler trainer
    trainer[classes] = Trainer(sampler=s,
                            discriminator=model,
                            criterion=criterion,
                            optimizer=optimizer,
                            scheduler=scheduler,
                            dataset=cyclicity,
                            k_samples=16,
                            seed=seed,
                            classes=classes)
    
    def penalty_cond(out, trainer, boundary_cls=classes):
        return out["probs"][0, classes].item() > 0.9
    def break_cond(out, trainer, boundary_cls=classes):
        return all([
            penalty_cond(out, trainer, boundary_cls),
            trainer.sampler.expected_m < 15
        ])
    
    # Train and log time
    trainer[2].train(2000, dynamic_penalty=budget, penalty_cond=penalty_cond, break_cond=break_cond, log_file=log_file_train)
    # Evaluate and record class probabilities and generated explanation graphs
    # trainer[2].evaluate(threshold=0.5, show=True, connected=True, log=True)
    trainer[2].evaluateAndLog(threshold=0.5, log_file_probs=log_file_probs)

if __name__ == '__main__':
    main()