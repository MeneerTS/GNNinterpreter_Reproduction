    ...
    try:
        ...
        with open(file_path, 'r') as file:
            ...
            for line in file:
                ...
                _, _, train_time_str = line.strip().split('\t')
...
def calculate_average_acc(log_path):
    ...
    if log_path[-5] == "g" or log_path[-5] == "y" or log_path[-5] == "w":
        num_classes += 1
    ...
    for class_num, df_class in dfs_by_class.items():
        ...
        print(f"Number of bad graphs for class {class_num}: {num_bad_graphs + num_empty_graphs_in_class}\n")
...
def main():
    # Class probabilities
    accuracy_paths = ["logs/class_probs_cyclicity_new.txt"]
    ...
    # Training time
    training_paths = ["logs/train_times_cyclicity_new.txt"]

    for _, file_path in enumerate(training_paths):
        average_train_time = calculate_average_train_time(file_path)
